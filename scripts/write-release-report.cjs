...
const candidateVersion=read('extension-unpacked/manifest.json').version;
const baselineSource=read('launch/qa/packages.json').baseline_source;
const baselineVersion=JSON.parse(execFileSync('git',['show',`${baselineSource}:extension-unpacked/manifest.json`],{encoding:'utf8'})).version;
const bundleHash=crypto.createHash('sha256').update(fs.readFileSync('extension-unpacked/bundle.js')).digest('hex');
...
Checked ${summary.checked}. Candidate technical version: **${candidateVersion}**. Product name: **HistoryOut**. Previous source release: **${baselineVersion}**, baseline \`${baselineSource}\`.
...
- Chrome's native unpacked installer updates the ${upgrade.previousVersion} baseline payload to ${candidateVersion}, opens exactly one internal update page, retains native history and exact prior preferences, and does not repeat the lifecycle page after a same-version restart. This is a real local installer transition, not a simulated event; it is not a signed Chrome Web Store update.
...
- No preview/export page network uploads or runtime exceptions were observed. Install/update pages load their interface locally from the extension package, separate from history processing.
...
- [Native ${upgrade.previousVersion} to ${candidateVersion} upgrade](./native-update.json)
