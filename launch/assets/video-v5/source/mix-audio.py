...
SR=48000
DURATION=json.loads((ROOT/'source/voice-metrics.json').read_text())['duration']
N=int(SR*DURATION)
...
for i,start in enumerate(np.arange(.4,DURATION-.5,.75)):
 ...
 idx=int(start*SR);pan=.25+.5*(i%3)/2;end=min(N,idx+len(t));score[idx:end,0]+=tone[:end-idx]*(1-pan);score[idx:end,1]+=tone[:end-idx]*pan
