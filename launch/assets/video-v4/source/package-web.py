...
metadata={'title':'Export your browser history as CSV, JSON or HTML with HistoryOut','duration_seconds':duration,'width':video['width'],'height':video['height'],'fps':video['r_frame_rate'],'video_codec':video['codec_name'],'audio_codec':audio['codec_name'],'audio_channels':audio['channels'],'sample_rate':int(audio['sample_rate']),'bytes':mp4.stat().st_size,'sha256':hashlib.sha256(mp4.read_bytes()).hexdigest(),'mp4':mp4.name,'poster':'historyout-poster.png','captions_srt':'historyout-captions.srt','captions_vtt':'historyout-captions.vtt','transcript_text':canonical,'timed_captions':captions,'speech_start':.2,'speech_end':round(.2+voice['speech_seconds'],3),'tail_seconds':round(duration-.2-voice['speech_seconds'],3),'words_per_minute':63/voice['speech_seconds']*60,'maximum_aligned_word_gap_seconds':round(max(b['start']-a['end'] for a,b in zip(words,words[1:])),3),'maximum_detected_voice_silence_seconds':max(gaps) if gaps else 0,'loudness_integrated_lufs':float(loudness['input_i']),'true_peak_dbtp':float(loudness['input_tp']),'loudness_range_lu':float(loudness['input_lra']),'mix_sample_peak_before_encode':metrics['peak'],'click_count':metrics['click_count'],'voice':'The prior locally generated creator-reference narration is reused sample-for-sample, with 1.8 seconds of brief breaths inserted between words; no additional speed or pitch change.','reference_url':'https://www.youtube.com/watch?v=8k0lNtIhPWs','verification':'All spoken words independently verified by local ASR in isolated voice and complete mix. Visual frames inspected. Human perceptual listening is not claimed.','history':'Actual working interface, real downloaded exports, fictional QA browsing data. Coffee promotion is hidden only through capture-page CSS; extension source is unchanged.','music':'Original synthesized ambient bed, no borrowed samples','clicks':'Original synthesized click effects synchronized to actual UI actions','brand':'Original HistoryOut wordmark and icons; no public version suffix','end_card':'Export browser history. CSV · JSON · HTML. Free. Local. No account. Get HistoryOut free.','final_card_start':voice['final_card_start'],'final_card_total_seconds':duration-voice['final_card_start'],'music_fade_start':26.0,'music_fade_end':29.0,'transition_seconds':.3,'added_breaths_seconds':voice['added_breaths_seconds'],'production_bundle_sha256':hashlib.sha256((ROOT.parents[2]/'extension-unpacked/bundle.js').read_bytes()).hexdigest()}
...
(ROOT/'source/silence-check.txt').write_text('\n'.join(line.rstrip() for line in silence.splitlines())+'\n')
subprocess.run(['ffmpeg','-y','-loglevel','error','-ss','3.8','-i',str(mp4),'-frames:v','1','-update','1',str(ROOT/'historyout-poster.png')],check=True)
frames=[21,114,300,339,408,507,588,720,798,864]
select='+'.join(f'eq(n,{n})' for n in frames)
subprocess.run(['ffmpeg','-y','-loglevel','error','-i',str(mp4),'-vf',f"select='{select}',scale=640:360,tile=2x5",'-frames:v','1','-update','1',str(ROOT/'review-contact-sheet.png')],check=True)
