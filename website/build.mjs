...
const product = JSON.parse(await readFile(path.join(root, 'content/product.json'), 'utf8'));
...
export async function build() {
  ...
  for (const page of pages) {
    ...
    const content = page.body.replaceAll('{{CTA}}', cta).replaceAll('{{STORE}}', store).replaceAll('{{SUPPORT}}', support).replaceAll('{{FEEDBACK}}', product.feedback).replaceAll('{{CONTACT}}', product.contactEmail).replaceAll('{{ROLLOUT}}', released ? '' : `<p class="rollout-note">Version ${escape(config.latestVersion)} is being prepared. The store currently offers version ${escape(config.storeVersion)}.</p>`);
    const robots = indexable && page.indexable !== false ? 'index,follow,max-image-preview:large,max-video-preview:-1' : 'noindex,follow';
    const note = !released && !['/welcome/','/changelog/'].includes(page.route) ? `<div class="release-note">A smoother start for HistoryOut. <a href="/changelog/">See what’s changing →</a><span>Version ${escape(config.latestVersion)} is being prepared. Store version: ${escape(config.storeVersion)}.</span></div>` : '';
    const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(page.title)}</title><meta name="description" content="${escape(page.description)}"><meta name="robots" content="${robots}"><link rel="canonical" href="${canonical}"><meta property="og:type" content="website"><meta property="og:site_name" content="HistoryOut"><meta property="og:title" content="${escape(page.title)}"><meta property="og:description" content="${escape(page.description)}"><meta property="og:url" content="${canonical}"><meta property="og:image" content="${origin}/assets/social-card.png"><meta property="og:image:width" content="1280"><meta property="og:image:height" content="720"><meta property="og:image:alt" content="HistoryOut. Export browser history as CSV, JSON or HTML."><meta name="twitter:card" content="summary_large_image"><meta name="theme-color" content="#2479e0"><meta name="google-site-verification" content="J9EsmoQ_afZAfv1vTpVgo5bu_xHrSMfvtdjN_fUqgrs"><link rel="icon" href="/favicon.ico"><link rel="stylesheet" href="/style.css"><script type="application/ld+json">${JSON.stringify(structuredData(page,content)).replaceAll('<','\\u003c')}</script><script src="/site.js" defer></script></head><body><a class="skip" href="#main">Skip to content</a>${note}<header class="site-header"><a class="brand" href="/" aria-label="HistoryOut home">${brand}</a><nav aria-label="Main navigation"><a href="/#how-it-works">How it works</a><a href="/guides/export-browser-history-to-html/">Guides</a><a href="/alternatives/">Compare</a><a href="${product.feedback}" target="_blank" rel="noopener noreferrer">Suggest a feature ↗</a><a class="nav-cta" href="${store}">Get it free ↗</a></nav></header><main id="main">${content}</main>${footer}</body></html>`;
